...
from olden_db.query import PlanningQueryService, UnknownBuildingError, UnknownFactionError


def main() -> None:
    data = load_default_game_data()
    service = PlanningQueryService(data)
    target = _representative_target(data)
    faction, sid, level = target.key.faction, target.key.sid, target.key.level
    ...
    expected = tuple(sorted(target.prerequisites, key=lambda key: (key.sid, key.level)))
    ...
    _check_errors(service, data)
    if hasattr(service, "data"):
        raise RuntimeError("PlanningQueryService still exposes public data state")
...
def _representative_target(data):
    buildings = sorted(
        (
            building
        ...
        key=lambda item: (item.key.faction, item.key.sid, item.key.level),
    )
    if not buildings:
...
def _check_errors(service: PlanningQueryService, data) -> None:
    try:
