...
class PlannerPresenter:
    """Coordinate target discovery, validated state, and view updates."""
    ...
    def on_level_changed(self, level: int) -> None:
        ...
        self._set_status(
            "Target selected: "
            f"{self._state.selected_faction}/"
            f"{self._state.selected_building_sid} "
            f"level {level}."
        )
    ...
    def on_generate_plan(self) -> None:
        ...
        self._set_status(
            "Build plan generation will be implemented in the next milestone."
        )
