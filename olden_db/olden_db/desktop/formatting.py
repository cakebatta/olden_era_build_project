...
def format_game_date(date: GameDate) -> str:
    return (
        f"Month {date.month}, Week {date.week}, Day {date.day} "
...
def format_target(building: BuildingLevel) -> str:
    return "\n".join(
        (
            ...
            "Constructed at game start: "
            f"{'Yes' if building.constructed_on_start else 'No'}",
...
def format_prerequisites(prerequisites: tuple[BuildingLevel, ...]) -> str:
    ...
    lines = [
        "Buildings already available at game start satisfy prerequisites "
        "but do not appear as construction actions.",
        "",
    ]
    for index, building in enumerate(prerequisites, start=1):
        status = (
            "Already constructed at game start"
            if building.constructed_on_start
            else "Requires construction"
        )
        lines.extend(
            (
