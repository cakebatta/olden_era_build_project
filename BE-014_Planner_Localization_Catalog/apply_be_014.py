...
def copy_payload(name: str, destination: Path) -> None:
    source = PAYLOAD / name
    if not source.is_file():
        raise FileNotFoundError(f"Missing package payload: {source}")
...
def main() -> None:
    ...
    copy_payload("planner_localization.py", PLANNER_LOCALIZATION)
    copy_payload("test_planner_localization_catalog.py", TEST)
    copy_payload("BE-014-IMPLEMENTATION-REPORT.md", REPORT)
    copy_payload("be014_runtime_verification.md", RUNTIME)
    patch_paths()
    patch_query()
    patch_docs()
    validate()
